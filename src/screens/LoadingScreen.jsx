  ...
  useEffect(() => {
    const run = async () => {
      try {
        ...
        // Build FormData — flow multipart historique. Le navigateur envoie
        // le WAV au backend (Railway, sans limite body), qui transcode en
        // MP3 256k AVANT de toucher à Supabase. Supabase ne voit donc QUE
        // des MP3 de ~10 Mo — économe en egress et compatible Free comme
        // Pro. L'endpoint backend `POST /api/storage/sign-upload` reste
        // dispo côté `decode-api` mais n'est pas câblé ici (cf.
        // `docs/UPLOAD_DIRECT_PLAN.md` pour la décision et le plan futur).
