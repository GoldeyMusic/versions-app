...
- **Bug ouvert : crash `master + declared_genre`** — depuis le déploiement C.2 du 20 mai, le combo `uploadType='master'` + champ Genre rempli fait planter le pipeline (`status:error` au stade `measures_done` après ~105s, aucune entrée `analysis_cost_logs`, aucune fiche persistée). Cause exacte non identifiée à ce jour : les try/catch de `_analyze.js` devraient absorber tout throw lisible, donc l'erreur vient probablement d'une exception async non capturée OU d'un kill Railway (OOM, dyno timeout). **Mitigation déployée 2026-05-21 dans `versions-api/lib/claude.js`** : si le combo est détecté en entrée de `generateFiche`, on bascule silencieusement sur le path `shouldInferGenre=true` (declared_genre ignoré, inferred_genre Gemini reprend). Workaround pour l'utilisateur : choisir "Non, pas masterisé" + remplir le Genre. Reste à activer un log verbeux Railway sur la prochaine récurrence pour cerner la cause et retirer la mitigation. Impact prod observé sur 14 j = 0 utilisateur (à part David qui testait) — combo très peu utilisé tant que la plupart restent en `auto`/`mix`.
