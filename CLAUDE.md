...
- **Cocher `customer.subscription.updated` dans les Listened events du webhook Stripe** (dashboard.stripe.com → Developers → Webhooks → endpoint prod). Sans ce check, le nouveau handler cancel-intent du backend (`api/_billing.js`) ne sera jamais déclenché → pas de notif ops à la demande d'annulation.
