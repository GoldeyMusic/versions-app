...
- **Pages légales `/privacy` et `/terms`** — éditeur **Archipel Audio** depuis 2026-07-26 (SAS en cours d'immatriculation, www.archipelaudio.com, contact hello@archipelaudio.com — SIRET à ajouter à réception ; ex-Multicolorz SIRET 819 747 296), i18n FR/EN via `STRINGS.legal.{privacy,terms}`, layout partagé dans `components/LegalLayout.jsx` (helper `renderLegalInline` pour `**gras**` + `{email}` + `{publisherEmail}` + `{archipel}`). Sous-traitants simplifiés en une phrase pointant vers `contact@versions.studio` pour la liste détaillée. Liens "Confidentialité · CGU" en bas de la landing.
- **Byline « by Archipel Audio » (2026-07-26)** — composant partagé `components/BrandByline.jsx` (une lettre par span, distribuées en flex space-between pour faire STRICTEMENT la largeur du wordmark VERSiONS) + classes `.brand-col`/`.brand-byline` dans MockupStyles. Montée sous le wordmark de toutes les topbars/brands : landing, pricing, plugin, légales, db-topbar (App.jsx), sidebar, page exemple, LoadingScreen. Footer landing : ligne « Versions est édité par Archipel Audio » (`.lp-footer-publisher`).
...
- **Ajouter le SIRET d'Archipel Audio** dans les 4 blocs éditeur de `strings.js` (CGU + privacy, FR + EN) dès réception — remplacer « en cours d'immatriculation en France » par « immatriculée en France sous le SIRET … ».
